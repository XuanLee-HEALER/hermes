...
#[tokio::main]
async fn main() {
    download_newest_tracker().await.unwrap()
}

async fn download_newest_tracker() -> Result<(), AnyError> {
    ...
    if let Ok(last_update) = update_time_record().await
        && last_modified <= last_update
    {
        return Err(Error::NoUpdate.into());
    } else {
        let mut f = File::create_new(UPDATE_TIME_RECORD).await?;
        f.write_all(format!("{}", last_modified.format(TIME_FMT)).as_bytes())
            .await?;
    }
...
mod tests {
    ...
    use crate::{TIME_FMT, UPDATE_TIME_RECORD, is_file, update_time_record};
